...
class App(ctk.CTk):
    def __init__(self):
        ...
        bg_image = ctk.CTkImage(Image.open("BuildedImages/background.png"), size=(1920, 1080))
        ...
        switch_button = ctk.CTkSwitch(self, text= "Mode", command= lambda self=self: self.mode_switch())
        switch_button.place(x=0, y=0)

        for file in files_list:
            self.openbutton.append(OpenButton(self, fname = file, row=len(self.openbutton), opened_frame = opened_frame))
...
# loads all buttons on the frame
    def load_buttons(self):
        ...
        with open(f"Saves/{self.file_n}") as file:
            ...
            for line in lines:
                button = Click_Button(master = self, line = line, font = button_font, row=_row, column=_column)
                _column += 1
                if _column == 5:
                    _column = 0
                    _row +=1
